...
let parseV lr =
  match !lr with
     (h::t) when h >= '0' && h <= '9' ->
       lr := t;
       Number ((int_of_char h) - (int_of_char '0'))
...
parseT lr =
  let x = parseU lr in
    match !lr with
       ('*'::t) -> lr := t; Times (x, parseT lr)
     | ('/'::t) -> lr := t; Divide (x, parseT lr)
     | _ -> x
...
parseS lr =
  let x = parseT lr in
    match !lr with
       ('+'::t) -> lr := t; Plus (x, parseS lr)
     | ('-'::t) -> lr := t; Minus (x, parseS lr)
     | _ -> x;;

let parse_expr s =
  let lr = ref (string_to_list s) in  (* "lr" == "list ref" *)
    let expr = parseS lr in
      if !lr = []
        then expr
        else raise
...
let rec eval = function
    Number n -> n
  | Plus (m, n) -> (eval m) + (eval n)
  | Minus (m, n) -> (eval m) - (eval n)
  | Times (m, n) -> (eval m) * (eval n)
  | Divide (m, n) -> (eval m) / (eval n);;
